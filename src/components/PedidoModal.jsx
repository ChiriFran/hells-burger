...
export default function PedidoModal({ mesa, cerrarModal }) {
  ...
  const handleCrear = async () => {
    if (submitting || lockRef.current || mesa.estado !== "libre" || productos.length === 0) return;
    ...
    try {
      const productosFinal = productos.map((p) => ({
        ...p,
        nombre: buildNombreConExtras(p), // 🔥 persistir nombre final
  ...
  const handleCerrar = async () => {
    if (submitting || lockRef.current || mesa.estado !== "ocupada" || !mesa.pedidoActual) return;
  ...
  // ===================== FILTROS =====================
  const categorias = ["todas", ...new Set(productosDisponibles.map((p) => p.categoria))];
      ...
      <div className="pedido-modal">
        ...
        <div className="modal-body">
                  ...
                  <li key={i} className="pedido-item">
                    <div className="item-info">
                      ...
                      <span className="item-meta">
                        {p.cantidad} × ${p.usarDoble && p.precioDoble ? p.precioDoble : p.precio}
                      </span>
                      ...
                      {p.precioDoble && (
                        <label className="doble-switch">
                          <input
                            type="checkbox"
                            checked={p.usarDoble}
                            onChange={(e) => updateProducto(i, { usarDoble: e.target.checked })}
                          />
                          Doble
                        </label>
                      )}
                      ...
                      <details className="extras-box">
                        ...
                        <div className="extra-row">
                          Carne extra 🥩
                          <button onClick={() => updateExtras(i, { carne: Math.max((p.extras?.carne || 0) - 1, 0) })}>−</button>
                          <span>{p.extras?.carne || 0}</span>
                          <button onClick={() => updateExtras(i, { carne: (p.extras?.carne || 0) + 1 })}>+</button>
                        </div>
                        ...
                        <label>
                          <input
                            ...
                            onChange={(e) => updateExtras(i, { cheddar: e.target.checked })}
                          />
                        ...
                        <label>
                          <input
                            ...
                            onChange={(e) => updateExtras(i, { bacon: e.target.checked })}
                          />
                    ...
                    <div className="item-right">
                      <span className="item-subtotal">${calcularSubtotal(p)}</span>
          ...
          <div className="pedido-form-card">
            ...
            <button className="btn-secundario" onClick={() => setShowProductos(true)}>
              📦 Elegir de productos
            </button>

            <div className="pedido-form">
              <input placeholder="Producto" value={nombre} onChange={(e) => setNombre(e.target.value)} />
              <input type="number" placeholder="Precio" value={precio} onChange={(e) => setPrecio(e.target.value)} />
              <input type="number" min="1" value={cantidad} onChange={(e) => setCantidad(e.target.value)} />

              <button onClick={agregarProducto} disabled={submitting}>➕ Agregar</button>
            </div>
          </div>
        </div>
        ...
        <div className="acciones">
          {mesa.estado === "libre" && (
            <button className="btn-primario" onClick={handleCrear} disabled={productos.length === 0 || submitting}>
              📝 Crear Pedido
            </button>
          )}
          ...
          {mesa.estado === "ocupada" && (
            <>
              <div className="medio-pago">
                <button className={medioPago === "efectivo" ? "activo" : ""} onClick={() => setMedioPago("efectivo")}>💵 Efectivo</button>
                <button className={medioPago === "otro" ? "activo" : ""} onClick={() => setMedioPago("otro")}>💳 Otro</button>
              </div>

              <button className="btn-secundario" onClick={handleCerrar} disabled={submitting}>
                ✅ Cerrar Pedido
              </button>
            </>
          )}
        ...
        <div className="pedido-overlay">
          <div className="pedido-modal">
            <div className="modal-header">
              ...
              <button className="btn-cerrar" onClick={() => setShowProductos(false)}>✖</button>
            </div>
            ...
            <div className="modal-body">
              ...
              {Object.entries(productosPorCategoria).map(([categoria, items]) => (
                <div key={categoria}>
                  ...
                  <ul className="pedido-lista">
                    {items.map((p) => (
                      <li key={p.id} className="pedido-item" style={{ cursor: "pointer" }} onClick={() => agregarProductoDirecto(p)}>
                        <div>
